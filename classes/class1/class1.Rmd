...
```{r}
# Load data
setwd("/Users/lukaskubiena/Desktop/Methods02/resources_class2")
hibbs <- read.table("classes/data/ElectionsEconomy/data/hibbs.dat", header = TRUE)
...
my_simulated_data <- rnorm(1000, 200, 30)

df <- data.frame(simulated_data = my_simulated_data)
...
That it should follow a normal distribution.
...
```{r}
ggplot(df, aes(x=simulated_data)) + geom_histogram()
```
...
```{r}
mean(df$simulated_data)
sd(df$simulated_data)
```
...
error <- rnorm(100, mean = 0, sd = 3.9) 
y <- 46.3 + 3.0 * x + error

df <- data.frame(x = x, y = y)

plot(y ~ x, df, main = "Simulated Data with Regression Line", xlab = "x", ylab = "y")
...
Forecasting election trends based on household income for the coming election cycle.
